...
### Added
- Sector presets: `--preset startup-tech` on `ptbot sweep:auto` loads a curated list of 38 tech startup verticals (FinTech, AI/ML, Cybersecurity, SaaS, CleanTech, and more) so one command progressively builds comprehensive coverage across the full startup ecosystem. `ptbot db:coverage [--preset startup-tech]` shows a coverage table (sector × years covered/missing/deals) so you can see gaps before the next sweep. Ships with `startup-sectors.toml` for TOML-based sweep runners.
- Cloud agent safeguards (cloud-safeguards): `ptbot cloud kill-all [--dry-run]` kills every active run in the registry in one command — the nuclear firedrill recovery option. WIP cap blocks new sweep dispatches when active cloud run count meets `--max-active` (default: 10). Timeout watchdog daemon thread kills runs that have exceeded `timeout × 1.5` seconds without reaching a terminal state.
