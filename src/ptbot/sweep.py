...
def run_sweep(
    ...
    *,
    ...
    """Run the sweep: iterate all (market × window) combinations sequentially.

    Combinations already present in the database are skipped automatically.
    ...
    try:
        for market in config.markets:
            for start_date, end_date in windows:
                ...
                if _db.query_run_exists(
                    conn, market.sector, market.geography, start_date, end_date
                ):
                ...
                output_dir = (
                    output_base / slug(market.sector) / slug(market.geography) / start_date[:4]
                )
                run_pipeline(
                    params,
                    output_dir,
                ...
                completed += 1
                # Re-open connection so the newly inserted run is visible
                conn.close()
                conn = _db.open_db(db_path)
    ...
    if dry_run:
        print(f"[sweep] dry-run complete — {total - skipped} would run, {skipped} skipped")
    else:
        print(f"[sweep] complete — {completed} run(s), {skipped} skipped")
